...
#     input_choice - choice of data to display, given by the user,
...
map <- function(data = NA, world_map = get.world.map(), input_choice) {
  ...
  plot_map <- ggplot() + 
    geom_polygon_interactive(data = subset(world_map), color = 'white', size = 0.2,
...
confirmed_total_map <-  function(user_input = "confirmed", 
  ...
  # Use ggplot to plot the map
  plot_map <- ggplot() + 
    geom_polygon_interactive(data = subset(world_map_2), color = 'white', size = 0.2,
